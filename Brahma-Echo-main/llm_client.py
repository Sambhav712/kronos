...
class UnifiedAIClient:
    ...
    def reload_settings(self):
        try:
            ...
            with open(SETTINGS_PATH, "r", encoding="utf-8") as f:
                data = json.load(f)
            self._provider = data.get("default_ai_provider", "Groq")
    ...
    def chat(self, prompt: str, system: str = "You are a helpful assistant.", history: Optional[list[dict]] = None, model: Optional[str] = None, max_tokens: int = 4096, temperature: float = 0.7) -> str:
        ...
        if self._provider == "Local":
            ...
            if groq_client.is_configured():
                try:
                    return groq_client.chat(prompt, system, history, model, max_tokens, temperature)
                except Exception as e:
                    logger.warning(f"[LLM Client] Groq failed, falling back to OpenRouter: {e}")
    ...
    def chat_json(self, prompt: str, system: str = "Return ONLY valid JSON.", model: Optional[str] = None, max_tokens: int = 4096) -> dict:
        ...
        if self._provider == "Local":
            ...
            if groq_client.is_configured():
                try:
                    return groq_client.chat_json(prompt, system, model, max_tokens)
                except Exception as e:
                    logger.warning(f"[LLM Client] Groq JSON failed, falling back to OpenRouter: {e}")
    ...
    def vision(self, prompt: str, image_b64: str, mime: str = "image/png", system: str = "Analyze the image.", model: Optional[str] = None, max_tokens: int = 1024) -> str:
        ...
        if self._provider == "Local":
            ...
            if groq_client.is_configured():
                try:
                    return groq_client.vision(prompt, image_b64, mime, system, model, max_tokens)
                except Exception as e:
                    logger.warning(f"[LLM Client] Groq vision failed, falling back to OpenRouter: {e}")
    ...
    def vision_from_file(self, prompt: str, image_path: str, system: str = "Analyze the image.", model: Optional[str] = None, max_tokens: int = 1024) -> str:
        ...
        if self._provider == "Local":
            ...
            if groq_client.is_configured():
                try:
                    return groq_client.vision_from_file(prompt, image_path, system, model, max_tokens)
                except Exception as e:
                    logger.warning(f"[LLM Client] Groq vision_from_file failed, falling back to OpenRouter: {e}")
    ...
    def multi_turn(self, messages: list[dict], model: Optional[str] = None, max_tokens: int = 4096, temperature: float = 0.7) -> str:
        ...
        if self._provider == "Local":
            ...
            if groq_client.is_configured():
                try:
                    return groq_client.multi_turn(messages, model, max_tokens, temperature)
                except Exception as e:
                    logger.warning(f"[LLM Client] Groq multi_turn failed, falling back to OpenRouter: {e}")
