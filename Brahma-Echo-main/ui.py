...
try:
    import importlib
    _qweb = importlib.import_module("PyQt6.QtWebEngineWidgets")
    QWebEngineView = getattr(_qweb, "QWebEngineView")
...
class RemoteKeyOverlay(QWidget):
    ...
    def _update_qr(self, url: str) -> None:
        ...
        try:
            ...
            buf = BytesIO()
            try:
                getattr(img, "save")(buf, format="PNG")
            except TypeError:
                getattr(img, "save")(buf)
            pix = QPixmap()
...
class WorkspaceSidebar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._title = QLabel("BRAHMA ECHO WORKSPACE")
    ...
    def _build_chat_tab(self) -> QWidget:
        ...
        self._input.setPlaceholderText("Message Brahma Echo...")
...
class InlineChatWorkspace(QFrame):
    ...
    def _build_chat_tab(self) -> QWidget:
        ...
        self._footer_status = QLabel("Brahma Echo is ready")
    ...
    def append_log(self, text: str):
        ...
        elif low.startswith("brahma echo:"):
...
class FloatingLauncher(QWidget):
    ...
    def _apply_state_style(self):
        self.setToolTip(
            f"Brahma Echo ({self._status_line})\n"
    ...
    def _show_menu(self, global_pos):
        ...
        open_full = QAction("Open Brahma Echo (Full App)", self)
    ...
    def _apply_state(self, state: str):
        ...
        if hasattr(self, "_status_badge"):
            badge_map = {
                ...
                "SPEAKING":   ("◈ SPEAKING",   "#ffb300", "rgba(255, 179, 0, 0.5)",  "rgba(255, 179, 0, 0.12)"),
        ...
        if hasattr(self, "_input"):
            ph_map = {
                ...
                "SPEAKING": "Brahma Echo is responding...",
            ...
            self._input.setPlaceholderText(ph_map.get(state, "Ask Brahma Echo anything..."))
        ...
        if hasattr(self, "_inline_workspace") and hasattr(self._inline_workspace, "_footer_status"):
            ...
            self._inline_workspace._footer_status.setText(foot_map.get(state, "Brahma Echo is online"))
...
class SystemConnectivityPage(QWidget):
    ...
    def _build_left_column(self):
        ...
        current_provider = self._load_app_settings().get("default_ai_provider", "Groq")
        if current_provider == "Groq":
            self._default_provider.setCurrentText("Groq")
        elif current_provider in {"Gemini", "Google Gemini"}:
    ...
    class IGLoginWorker(__import__('PyQt6').QtCore.QThread):
        ...
        def run(self):
            try:
                import importlib
                try:
                    _ig = importlib.import_module("instagrapi")
                    _ig_exc = importlib.import_module("instagrapi.exceptions")
                    Client = _ig.Client
                    TwoFactorRequired = _ig_exc.TwoFactorRequired
                    ChallengeRequired = _ig_exc.ChallengeRequired
                except ImportError:
                    raise ImportError("The 'instagrapi' package is not installed. Please install it using 'pip install instagrapi'.")
                import json
                from pathlib import Path
                cl = Client()
    ...
    def _test_provider(self, setting_key: str):
        if setting_key == "groq":
            msg = "Groq key detected." if self._load_api_defaults().get("groq_api_key") else "Groq key missing."
        elif setting_key == "gemini":
    ...
    def refresh(self):
        ...
        try:
            ...
            p_val = app.get("default_ai_provider", "Groq")
            if p_val == "Groq":
                self._default_provider.setCurrentText("Groq")
            elif p_val in {"Gemini", "Google Gemini"}:
                self._default_provider.setCurrentText("Google Gemini")
            elif p_val == "Local":
                self._default_provider.setCurrentText("Local")
            else:
