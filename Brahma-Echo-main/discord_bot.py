...
logger = logging.getLogger("kronos.discord")
...
def _load_api_keys() -> dict:
    ...
    try:
        ...
        pass
    import os
    if not res.get("gemini_api_key"):
        res["gemini_api_key"] = (os.environ.get("GEMINI_API_KEY", "") or os.environ.get("GOOGLE_API_KEY", "")).strip()
    if not res.get("openrouter_api_key"):
        res["openrouter_api_key"] = os.environ.get("OPENROUTER_API_KEY", "").strip()
    return res
    ...
    async def _send_channel_message(self, channel, role: str, text: str):
        ...
        if isinstance(channel, int):
            try:
                ...
                return
        prefix = "Kronos" if role == "assistant" else "You" if role == "user" else "System"
            ...
            async def on_message(message):
                ...
                if self._app_submitter is not None:
                    ...
                    try:
                        ...
                        await message.reply(
                            "I couldn’t hand that command to Kronos.",
