...
def _get_api_key() -> str:
    try:
        ...
        pass
    env_key = (os.environ.get("GEMINI_API_KEY", "") or os.environ.get("GOOGLE_API_KEY", "")).strip()
    if env_key:
        return env_key
    return ""
...
def _ig_gemini_reply(username: str, text: str) -> str:
    ...
    # ── Stage 1: Groq (primary — fastest) ──
    try:
        groq_client.reload_key()
        if groq_client.is_configured():
            reply = groq_client.chat(prompt, system=system_prompt)
            if reply:
                return reply
    except Exception as ge:
        print(f"[InstagramChat] ⚠️ Groq primary failed: {ge}")
    
    # ── Stage 2: Gemini (secondary) ──
    try:
        client = genai.Client(
            api_key=_get_api_key(),
            http_options={"api_version": "v1beta"},
        ...
        try:
            ...
            if groq_client.is_configured():
                rewritten = groq_client.chat(
                    prompt,
                    system="You are a friendly assistant helping a user rewrite their draft reply for a chat message. Return only the rewritten reply text.",
                )
                if rewritten:
        ...
        try:
            ...
            if not reply:
                try:
                    groq_client.reload_key()
                    if groq_client.is_configured():
                        reply = groq_client.chat(request_text, system=_system)
                except Exception as e:
                    print(f"[BRAHMA ECHO] ⚠️ Groq primary failed: {e}")

            # ── Stage 2: Gemini (secondary) ──
            if not reply and gemini_first:
                try:
            ...
            # ── Stage 3: OpenRouter (last resort) ──
            if not reply:
                try:
