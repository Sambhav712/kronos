...
PLANNER_PROMPT = """You are the planning module of KRONOS AI, a personal AI assistant.
...
def create_plan(goal: str, context: str = "") -> dict:
    ...
    # ── Stage 1: Groq (primary — fastest via LPU & dense llama-3.3-70b-versatile) ──
    try:
        if groq_client:
            groq_client.reload_key()
            if groq_client.is_configured():
                plan = groq_client.chat_json(
                    prompt=user_input,
                ...
                if plan and isinstance(plan.get("steps"), list):
                    ...
                    print(f"[Planner] ✅ Plan (via Groq llama-3.3-70b-versatile): {len(plan['steps'])} steps")
                    for s in plan["steps"]:
                        print(f"  Step {s.get('step')}: [{s.get('tool')}] {s.get('description')}")
                    return plan
    except Exception as e:
        print(f"[Planner] ⚠️ Groq planning error: {e}")
    ...
    try:
        ...
        print(f"[Planner] ⚠️ Gemini initialization error: {e}")
    ...
    try:
        ...
        print(f"[Planner] ✅ Plan (via Gemini): {len(plan['steps'])} steps")
        for s in plan["steps"]:
...
Create a REVISED plan for the remaining work only. Do not repeat completed steps."""
            ...
            if groq_client.is_configured():
                ...
                if plan and isinstance(plan.get("steps"), list):
                    for step in plan.get("steps", []):
                        _rewrite_generated_step(step, goal)
                    print(f"[Planner] 🔄 Revised plan (via Groq llama-3.3-70b-versatile): {len(plan['steps'])} steps")
                    return plan
    except Exception as e:
        print(f"[Planner] ⚠️ Groq replan error: {e}")
    ...
    try:
        ...
        print(f"[Planner] 🔄 Revised plan (via Gemini): {len(plan['steps'])} steps")
        return plan
    except Exception as e:
