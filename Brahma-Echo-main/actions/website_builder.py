...
try:
    ...
    USER_DATA_DIR = Path(os.getenv("LOCALAPPDATA", os.path.expanduser("~"))) / "KRONOS AI"

BASE_DIR = Path(__file__).resolve().parent.parent
SETTINGS_PATH = USER_DATA_DIR / "config" / "app_settings.json"
FALLBACK_SETTINGS_PATH = BASE_DIR / "config" / "app_settings.json"
API_CONFIG_PATH = USER_DATA_DIR / "config" / "api_keys.json"
FALLBACK_API_CONFIG_PATH = BASE_DIR / "config" / "api_keys.json"
...
def _load_settings() -> dict[str, Any]:
    ...
    return _load_json(FALLBACK_SETTINGS_PATH)


def _get_api_key() -> str:
    for path in (API_CONFIG_PATH, FALLBACK_API_CONFIG_PATH):
        if path.exists():
            key = str(_load_json(path).get("gemini_api_key", "")).strip()
            if key:
                return key
    return (
...
def _is_safe_workspace(path: Path) -> bool:
    try:
        resolved = path.resolve()
        base_resolved = BASE_DIR.resolve()
        # Refuse to write inside application source folder
        if resolved == base_resolved or base_resolved in resolved.parents or resolved in base_resolved.parents:
            return False
        # Refuse drive roots (e.g. C:\ or /)
        if len(resolved.parts) <= 1:
            return False
        # Refuse system directories
        ...
        if resolved == Path(prog_files).resolve() or Path(prog_files).resolve() in resolved.parents:
            return False
        return True
    except Exception:
        return False
...
def _selected_workspace(parameters: dict[str, Any]) -> Path:
    requested = str(
    ...
    if requested:
        ...
        configured = str(
            settings.get("website_builder_workspace")
        ...
        if configured:
            ...
            path = DEFAULT_WEBSITES_DIR

    if path.is_file():
        path = path.parent

    if not _is_safe_workspace(path):
        raise ValueError(f"Target workspace folder '{path}' is unsafe or inside application source.")
...
def _resolve_project_dir(parameters: dict[str, Any]) -> Path:
    ...
    site_name = _sanitize_text(
        str(parameters.get("site_name") or parameters.get("title") or ""),
        "",
    )
    if not site_name:
        brief = str(parameters.get("brief") or parameters.get("description") or parameters.get("prompt") or "Website")
        site_name = _infer_site_title(brief, "Website")

    project_dir = workspace / _safe_slug(site_name)
    if not _is_safe_workspace(project_dir):
        raise ValueError(f"Target project folder '{project_dir}' is unsafe or inside application source.")
...
def _fallback_spec(parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    site_name = _sanitize_text(
        str(parameters.get("site_name") or parameters.get("title") or "KRONOS  Project"),
        "KRONOS  Project",
    )
...
"""
        js = """
const STORAGE_KEY = 'KRONOS .groceries.items';
...
Hard rules:
- Do not mention KRONOS , Codex, or any assistant branding in the site content.
...
"""
    ...
    if thread.is_alive() or not isinstance(bundle, dict):
        try:
            ...
            raw_response = unified_client.chat(prompt)
            files = _parse_code_bundle_text(raw_response)
...
def _normalize_page(page: dict[str, Any], site_name: str, brief: str, idx: int) -> dict[str, Any]:
    ...
    hero_data = {
        "eyebrow": _sanitize_text(str(hero.get("eyebrow") or ("Built with KRONOS " if idx == 0 else "Project Page")), "Built with KRONOS "),
    ...
    return f"""
      <div class="workspace-shell">
        <aside class="workspace-sidebar">
          <div class="sidebar-brand">KRONOS  Workspace</div>
...
<body id="top">
  ...
  <header class="topbar">
    <div class="brand">
      ...
      <div>
        ...
        <span>{_html(tagline or "Generated with KRONOS ")}</span>
      </div>
    </div>
  ...
  <footer class="footer reveal">
    <div>
      <strong>{_html(spec.get("site_name") or "Website")}</strong>
      <p>{_html(tagline or "Generated with KRONOS ")}</p>
    </div>
...
<body id="top">
  ...
  <main class="layout-{_html(layout_mode)}">
    <section class="hero hero--{_html(layout_mode)} reveal">
      <div class="hero-copy">
        <p class="eyebrow">{_html(hero.get("eyebrow") or "Built with KRONOS ")}</p>
        <h1>{_html(hero.get("title") or title)}</h1>
        <p class="lead">{_html(hero.get("subtitle") or tagline)}</p>
        <div class="hero-actions">
...
def website_builder(
    ...
    try:
        ...
        return f"Cannot initialize project directory: {exc}"

    if player:
        try:
            player.write_log(f"[WebsiteBuilder] Preparing website into {project_dir}")
        except Exception:
            pass

    # Safety confirmation check: if target directory exists and has non-website user files
    existed = project_dir.exists() and any(project_dir.iterdir())
    if existed and not params.get("overwrite", False) and not params.get("confirm", False):
        existing_filenames = {f.name for f in project_dir.iterdir() if f.is_file()}
        known_website_files = {"index.html", "styles.css", "script.js", "site-data.json", "README.md"}
        custom_files = existing_filenames - known_website_files
        if custom_files:
            return (
                f"Directory '{project_dir.name}' exists and contains custom files ({', '.join(sorted(custom_files)[:3])}). "
                f"Please confirm overwrite to proceed safely."
            )

    # Snapshot existing files for undo
    backup: dict[str, str] = {}
    if existed:
        for existing_file in project_dir.glob("**/*"):
            if existing_file.is_file():
                rel = str(existing_file.relative_to(project_dir))
                try:
                    backup[rel] = existing_file.read_text(encoding="utf-8", errors="ignore")
                except Exception:
                    pass
    ...
    def _undo() -> str:
        try:
            if not existed:
                ...
                return f"Removed created website directory '{project_dir.name}'."
            else:
                ...
                return f"Restored previous files in website directory '{project_dir.name}'."
        except Exception as exc:
            return f"Undo failed for '{project_dir.name}': {exc}"

    try:
        from core.undo import push_undo
        push_undo(f"build website {project_dir.name}", _undo)
    except Exception:
        pass

    preview_enabled = bool(params.get("preview", True))
    auto_open = bool(params.get("auto_open", True))

    if not preview_enabled:
        if player:
            try:
                player.write_log(f"[WebsiteBuilder] Website files written to {project_dir} (preview skipped)")
            except Exception:
                pass
        return f"Website built in {project_dir}."
    ...
    if auto_open:
        ...
        opened.append("preview server active")
    ...
    if speak:
        try:
            speak("Your website is ready and running in your browser, sir.")
        except Exception:
            pass
